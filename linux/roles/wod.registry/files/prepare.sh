...
if [[ -e /etc/kubernetes/downloads/$RKT_ACI_REGISTRY ]]; then
  ...
  if [[ -n "${REGISTRY_REMOTE:-}" ]]; then
    rkt --insecure-options=image fetch docker://$REGISTRY_REMOTE$REGISTRY_REPO_REGISTRY:$REGISTRY_VERSION_REGISTRY
    rkt image export $REGISTRY_REMOTE$REGISTRY_REPO_REGISTRY:$REGISTRY_VERSION_REGISTRY /etc/kubernetes/downloads/$REGISTRY_REPO_REGISTRY-$REGISTRY_VERSION_REGISTRY.aci
  else
