...
if [[ -e /etc/kubernetes/ssl/apiserver.pem ]]; then
  ...
  openssl x509 -req -in apiserver.csr -CA ca.pem -CAkey ca.key -CAcreateserial -out apiserver.pem -days 36500 -extensions v3_req -extfile master-apiserver.cnf
  echo 'apiserver.pem is ready!'
fi
...
if [[ -e /etc/kubernetes/ssl/admin.pem ]]; then
  ...
  openssl x509 -req -in admin.csr -CA ca.pem -CAkey ca.key -CAcreateserial -out admin.pem -days 36500 -extensions v3_req -extfile master-admin.cnf  
  echo 'admin.pem is ready!'
fi