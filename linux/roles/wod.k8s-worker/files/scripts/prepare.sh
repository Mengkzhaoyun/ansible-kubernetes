...
RKT_ACI_KUBELET="${RKT_ACI_KUBELET:-hyperkube-v1.14.1.aci}"
...
if [[ -e /etc/kubernetes/downloads/$RKT_ACI_KUBELET ]]; then
  ...
  if [[ -n "${REGISTRY_LOCAL:-}" ]]; then
    rkt --insecure-options=http,image fetch docker://$REGISTRY_LOCAL$REGISTRY_KUBELET_REPO:$REGISTRY_KUBELET_VERSION
    REGISTRY_LOCAL_RKT=$(echo $REGISTRY_LOCAL | sed 's/:/_/g')
    rkt image export $REGISTRY_LOCAL_RKT$REGISTRY_KUBELET_REPO:$REGISTRY_KUBELET_VERSION /etc/kubernetes/downloads/$REGISTRY_KUBELET_REPO-$REGISTRY_KUBELET_VERSION.aci
  else
